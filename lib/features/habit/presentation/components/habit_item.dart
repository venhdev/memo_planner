...
class HabitItemBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onLongPress: () {
        // context.go('/habit/detail/${habit.hid}', extra: habit);
        context.go('/habit/detail', extra: habit);
        ...
        startActionPane: ActionPane(
          ...
          children: [
            ...
            SlidableAction(
              borderRadius: BorderRadius.circular(12.0),
              onPressed: (context) {
                // > check if user is the owner of the habit then allow to edit
                // > else show message
                final currentUser = BlocProvider.of<AuthenticationBloc>(context).state.user;
                if (currentUser != null && currentUser.uid == habit.creator!.uid) {
                  showChooseEditTypeDialog(context: context);
                } else {
                  // show message
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('You are not the owner of this habit'),
                    ),
                  );
                }
              },
        ...
        endActionPane: ActionPane(
          ...
          children: [
            ...
            SlidableAction(
              borderRadius: BorderRadius.circular(12.0),
              onPressed: (context) {
                // > check if user is the owner of the habit then allow to delete
                final currentUser = BlocProvider.of<AuthenticationBloc>(context).state.user;
                if (currentUser != null && currentUser.uid == habit.creator!.uid) {
        ...
        child: Builder(builder: (context) {
          ...
          return ListTile(
            ...
            trailing: TouchableIcon(
              onTap: () {
                // context.go('/habit/detail/${habit.hid}');
                log('the habit pass to extra: $habit');
                context.go('/habit/detail', extra: habit);
              },
  ...
  void showChooseEditTypeDialog({
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Which one you want to edit?'),
          content: const Text(
